...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const savedCart = localStorage.getItem('lunchCart');
  if (savedCart) {
...
function addToCart(button) {
  ...
  localStorage.setItem('lunchCart', JSON.stringify(cart));
}
  ...
  if (cart.length === 0) {
      cartItemsDiv.innerHTML = `
          <div class="empty-cart-message">
              ...
              <p>Add some delicious lunch items!</p>
          </div>
      `;
  } else {
...
function clearCart() {
  ...
  if (confirm('Are you sure you want to clear your cart?')) {
      ...
      localStorage.removeItem('lunchCart');
  }
}
...
async function recordSale() {
  ...
  try {
      ...
      await db.collection('sales').add({
          ...
          status: 'completed',
          menuType: 'lunch' // Added to distinguish breakfast/lunch sales
      });
